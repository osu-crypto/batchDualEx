...
namespace libBDX{
   ...
   //inline void Enc(u8 i, block& dest, const block& src)
   //{
   //
   //
   //   mEnc[i]->ProcessData(dest, src, sizeof(block));
   //}
   block ClassicGarbledCircuit::GetNextRand()
   {
      block ret;
      AES128::EcbEncBlock(mSeedKey, _mm_set_epi64x(0, mNextRandIdx++), ret);
      return ret;
   }
...
#define Classic_GC_EVAL_INLINE
   ...
   void ClassicGarbledCircuit::Garble(const Circuit& cir, const block& seed)
   {
      ...
      AES128::EncKeyGen(seed, mSeedKey);
      mNextRandIdx = 0;

      mGlobalOffset = GetNextRand();
      ByteArray(mGlobalOffset)[0] |= 1; // make sure the bottom bit is a 1 for point-n-permutue

      for (u64 i = 0; i < cir.InputWireCount(); ++i)
            mWires.emplace_back(GetNextRand());
      ...
      for (const auto& gate : cir.Gates())
      {
         ...
         std::array<AES128::Key, 2> aKeys, bKeys;
   ...
   //
   //         block temp2, temp3;
   //         aGoldKeys[aIdx].ProcessData(ByteArray(temp2), ByteArray(wireValue), 16);
   //         bGoldKeys[bIdx].ProcessData(ByteArray(temp3), ByteArray(temp2), 16);
   //
